...
// Async actions
export function addToPlaylists(title) {
  return async (dispatch) => {
    const playlist = await playlistService.insert({ title, tracks: [] });
...
export function addTrackToPlaylist(playlistId, trackId) {
  return async (dispatch) => {
    ...
    const currentTracks = playlist.tracks.map((id) => id.toString());
    console.log(playlistId);
    console.log(playlist);

    if (!playlist || currentTracks.includes(trackId)) {
      return;
    }
