...
function handleCardInput() {
  ...
  // Megengedett max hosszúság
  if (card.value.length > 19) {
    card.value = card.value.slice(0, 19);
  }

  // Ha most írtunk be úgy, hogy kell a szóköz
  if (card.value.length === 4 || 
      card.value.length === 9 || 
      card.value.length === 14) {
    if (previousValue.length < card.value.length) {
      card.value += " "; 
    } else {
